...
@pytest.mark.asyncio
async def test_chat_crash_after_completed_ask_question_skips_chat_ask(
    monkeypatch,
    caplog,
):
    prompt_started = asyncio.Event()
    ...
    class CrashingAfterCompleteChat(FakeChatWindowController):
        def __init__(self):
            super().__init__()
            self.submission_task = None
            self.ask_release = asyncio.Event()
        ...
        async def run(self, on_submit, on_exit=None, on_interrupt=None):
            self.is_running = True
            self.submission_task = asyncio.create_task(on_submit("ask"))
            await asyncio.wait_for(engine.completed.wait(), timeout=1.0)
            await asyncio.sleep(0)
            if on_exit:
                result = on_exit()
                if hasattr(result, "__await__"):
                    await result
            raise RuntimeError("chat crashed after completed ask")
    ...
    engine = AskThenCompleteEngine()
    chat = CrashingAfterCompleteChat()
    monkeypatch.setattr(REPLEngine, "_run_prompt_session_loop", fake_prompt_loop)
    repl = REPLEngine(
        config=make_chat_config(enabled=True),
        chat_window_controller=chat,
...
async def test_permission_confirm_uses_active_chat_prompt_even_when_stdin_is_non_tty(
    ...
    monkeypatch.setattr(
        "myagent.permissions.controller.sys.stdin",
...
@pytest.mark.asyncio
async def test_permission_confirm_denies_explicit_repl_deny():
    permissions = PermissionController()
    chat = FakeChatWindowController(ask_response="d")
    active_chat_repl(
        chat,
        engine=SimpleNamespace(permissions=permissions),
    )
...
@pytest.mark.asyncio
async def test_permission_confirm_denies_invalid_repl_response():
    permissions = PermissionController()
    chat = FakeChatWindowController(ask_response="maybe")
    active_chat_repl(
        chat,
        engine=SimpleNamespace(permissions=permissions),
    )
