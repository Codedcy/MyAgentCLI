...
```mermaid
flowchart TD
    START([START]) --> LOAD[加载: 用户配置 → 项目配置 → 项目指导文件 → 记忆索引 → 环境感知]
    LOAD --> INPUT[读取用户输入]
...
│       用户消息 ↔ 模型响应 ↔ 工具调用/结果               │
├─────────────────────────────────────────────────────────┤
│  L4: Memory Index          ← 列出记忆索引，正文按需读取  │
├─────────────────────────────────────────────────────────┤
│  L3: Project Context       ← AGENTS.md, 配置, 环境感知  │
├─────────────────────────────────────────────────────────┤
...
| L4 Memory | 注入最多 200 条记忆索引，不自动加载正文 | 不加载（避免上下文污染） |
...
- **system**: L0（行为准则）+ L3（项目上下文）+ L4（记忆索引）+ L2 skills index
...
```bash
myagent                              # 新会话
myagent init                         # 生成项目根目录 AGENTS.md 模板
...
**启动时 / 每轮构建上下文时**: 读取项目级与用户级 MEMORY.md 索引 → 将最多 200 条记忆索引信息注入 L4（名称、类型、描述、路径）→ 不自动加载记忆正文

**对话中**: 模型根据 L4 索引判断是否需要某条记忆 → 使用索引中的路径通过 read 工具读取正文 → memory_write 写入/更新记忆 → 检查是否有覆盖同一事实的旧文件 → 更新而非重复创建 → 可删除过时记忆
...
4. 项目级 AGENT.md         .myagent/AGENT.md YAML frontmatter
5. 用户级 config.yaml      ~/.myagent/config.yaml
6. 用户级 AGENT.md         ~/.myagent/AGENT.md
7. 内置默认值              硬编码 fallback
```

项目根目录 `AGENTS.md` 属于 L3 Project Context 指导文件，不参与配置合并优先级；它影响模型收到的项目约定，但不直接修改运行时配置。
...
recently completed tool when no tool is running. The compact display keeps
routine tool execution visible without consuming the conversation space.
